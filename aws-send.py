...
def load_rotating_headers():
    """Load rotating headers from headers.txt"""
    headers_list = []
    try:
...
def ConnectSMTP(smtpserv,port,useauth,user,password,receivers,sender,usessl,msginfo):
    ...
    Args:
        smtpserv: SMTP server address
        port: SMTP port number
        useauth: Whether to use authentication
        user: Username for authentication
        password: Password for authentication
        receivers: Email recipient(s)
        sender: Sender email address
        usessl: Whether to use SSL/TLS
        msginfo: Email message content

    Returns:
        bool: True if email sent successfully, False otherwise
    """
    # ADVANCED DEBUGGING: Print all received parameters
    print(f"[DEBUG] Connecting to SMTP server...")
    print(f"[DEBUG] Server: {smtpserv}")
    print(f"[DEBUG] Port: {port} (type: {type(port)})")
    print(f"[DEBUG] useauth: {useauth} (type: {type(useauth)})")
    print(f"[DEBUG] user: '{user}' (length: {len(user) if user else 0})")
    print(f"[DEBUG] password: '{password[:10]}...' (length: {len(password) if password else 0})")
    print(f"[DEBUG] receivers: {receivers}")
    ...
    try:
        if port == 465:
            ...
            print(f"[DEBUG] Using port 465 with SMTP_SSL")
            with smtplib.SMTP_SSL(smtpserv, port, context=context) as server:
                print(f"[DEBUG] Connected, calling EHLO...")
                server.ehlo()
                print(f"[DEBUG] EHLO successful")
                
                if useauth:
                    print(f"[DEBUG] Authentication enabled, attempting login...")
                    print(f"[DEBUG] Login user: '{user}'")
                    try:
                        result = server.login(user, password)
                        print(f"[DEBUG] Login successful: {result}")
                    except Exception as login_error:
                        print(f"[DEBUG] Login FAILED: {login_error}")
                        raise
                else:
                    print(f"[DEBUG] Authentication DISABLED - this might be the problem!")
                
                print(f"[DEBUG] Attempting to send email from {sender} to {receivers}")
                server.sendmail(sender, receivers, msginfo)
                print(f"[DEBUG] Email sent successfully!")
                return True
        else:
            # Port 587 uses STARTTLS
            print(f"[DEBUG] Using port 587 with STARTTLS")
            with smtplib.SMTP(smtpserv, port) as server:
                print(f"[DEBUG] Connected, calling EHLO...")
                server.ehlo()
                print(f"[DEBUG] EHLO successful")
                
                # Start TLS if SSL is requested or if using port 587
                if usessl or port == 587:
                    print(f"[DEBUG] Starting TLS...")
                    server.starttls(context=context)
                    server.ehlo()
                    print(f"[DEBUG] TLS started successfully")
        ...
        error_msg = f"Authentication failed: {e.smtp_error.decode('utf-8', errors='ignore') if e.smtp_error else 'Invalid credentials'}"
        ...
        print(f"[DEBUG] TIMEOUT: Connection timeout")
        raise Exception("Connection timeout: SMTP server did not respond in time")
...
def Sendout(emails,smtpserv,port,its_authy,username,password,fromemail,sender,its_secure):
    ...
    try:
        try:
            # DEBUG: Show parameters before calling ConnectSMTP
            print(f"\n[DEBUG-SENDOUT] Preparing to send to: {emails}")
            print(f"[DEBUG-SENDOUT] SMTP Server: {smtpserv}")
            print(f"[DEBUG-SENDOUT] Port: {port}")
            print(f"[DEBUG-SENDOUT] Auth enabled: {its_authy}")
            print(f"[DEBUG-SENDOUT] Username: '{username}'")
            ...
            print(f"[DEBUG-SENDOUT] Exception caught: {e}")
